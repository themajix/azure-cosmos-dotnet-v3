...
namespace Microsoft.Azure.Cosmos
{
    ...
    internal sealed class ResourceFeedReader<T> : IEnumerable<T> where T : JsonSerializable, new()
    {
        ...
        public Task<FeedResponseCore<T>> ExecuteNextAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            return TaskHelper.InlineIfPossible(() => this.ExecuteNextAsyncInternal(cancellationToken), null, cancellationToken);
        }

        private async Task<FeedResponseCore<T>> ExecuteNextAsyncInternal(CancellationToken cancellationToken)
        {
