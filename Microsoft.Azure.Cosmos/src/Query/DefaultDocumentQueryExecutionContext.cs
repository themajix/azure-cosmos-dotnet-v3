...
namespace Microsoft.Azure.Cosmos.Query
{
    ...
    internal sealed class DefaultDocumentQueryExecutionContext : DocumentQueryExecutionContextBase
    {
        ...
        protected override async Task<FeedResponseCore<CosmosElement>> ExecuteInternalAsync(CancellationToken token)
        {
            ...
            return await BackoffRetryUtility<FeedResponseCore<CosmosElement>>.ExecuteAsync(
                async () =>
                {
                    this.fetchExecutionRangeAccumulator.BeginFetchRange();
                    ++this.retries;
                    Tuple<FeedResponseCore<CosmosElement>, string> responseAndPartitionIdentifier = await this.ExecuteOnceAsync(retryPolicyInstance, token);
                    FeedResponseCore<CosmosElement> response = responseAndPartitionIdentifier.Item1;
                    ...
                    if (!string.IsNullOrEmpty(response.ResponseHeaders[HttpConstants.HttpHeaders.QueryMetrics]))
                    {
                        this.fetchExecutionRangeAccumulator.EndFetchRange(
                            ...
                            response.Count, 
                            this.retries);
                        response = new FeedResponseCore<CosmosElement>(
                            response,
                            response.Count,
                            response.Headers,
        ...
        private async Task<Tuple<FeedResponseCore<CosmosElement>, string>> ExecuteOnceAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                FeedResponseCore<CosmosElement> feedRespose;
                ...
                else
                {
                    ...
                    if (ServiceInteropAvailable())
                    {
                        ...
                        FeedResponseCore<CosmosElement> response = await this.ExecuteRequestAsync(request, retryPolicyInstance, cancellationToken);
                ...
                return new Tuple<FeedResponseCore<CosmosElement>, string>(feedRespose, partitionIdentifier);
            }
        }
