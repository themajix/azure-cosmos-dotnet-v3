...
namespace Microsoft.Azure.Cosmos.Query
{
    ...
    internal sealed class DocumentProducer
    {
        ...
        /// </summary>
        private readonly AsyncCollection<TryMonad<FeedResponseCore<CosmosElement>>> bufferedPages;
        ...
        /// <summary>
        /// The callback used to take a <see cref="DocumentServiceRequest"/> and retrieve a page of documents as a <see cref="FeedResponseCore{T}"/>
        /// </summary>
        private readonly Func<DocumentServiceRequest, IDocumentClientRetryPolicy, CancellationToken, Task<FeedResponseCore<CosmosElement>>> executeRequestFunc;
        ...
        public DocumentProducer(
            ...
            Func<DocumentServiceRequest, IDocumentClientRetryPolicy, CancellationToken, Task<FeedResponseCore<CosmosElement>>> executeRequestFunc,
        ...
        {
            this.bufferedPages = new AsyncCollection<TryMonad<FeedResponseCore<CosmosElement>>>();
            ...
            try
            {
                ...
                using (DocumentServiceRequest request = this.createRequestFunc(this.PartitionKeyRange, this.backendContinuationToken, pageSize))
                {
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            FeedResponseCore<CosmosElement> feedResponse = await this.executeRequestFunc(request, retryPolicy, token);
                            ...
                            await this.bufferedPages.AddAsync(TryMonad<FeedResponseCore<CosmosElement>>.FromResult(feedResponse));
                        ...
                        catch (Exception exception)
                        {
                            ...
                            if (!shouldRetryResult.ShouldRetry)
                            {
                                ...
                                await this.bufferedPages.AddAsync(TryMonad<FeedResponseCore<CosmosElement>>.FromException(exceptionToBuffer));
        ...
        private async Task<bool> MoveNextPage(CancellationToken token)
        {
            ...
            TryMonad<FeedResponseCore<CosmosElement>> tryMonad = await this.bufferedPages.TakeAsync(token);
            FeedResponseCore<CosmosElement> feedResponse = tryMonad.Match<FeedResponseCore<CosmosElement>>(
                onSuccess: ((page) => 
                {
                    return page;
                }),
