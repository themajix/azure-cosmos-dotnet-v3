...
namespace Microsoft.Azure.Cosmos.Linq
{
    ...
    internal sealed class ChangeFeedQuery<TResource> : IDocumentQuery<TResource> where TResource : new()
    {
        ...
        public Task<FeedResponseCore<TResult>> ExecuteNextAsync<TResult>(CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        #region Private
        public Task<FeedResponseCore<TResult>> ReadDocumentChangeFeedAsync<TResult>(string resourceLink, CancellationToken cancellationToken)
        {
        ...
        private async Task<FeedResponseCore<TResult>> ReadDocumentChangeFeedPrivateAsync<TResult>(string link, IDocumentClientRetryPolicy retryPolicyInstance, CancellationToken cancellationToken)
        {
            using (DocumentServiceResponse response = await this.GetFeedResponseAsync(link, resourceType, retryPolicyInstance, cancellationToken))
            {
                ...
                if (response.ResponseBody != null && response.ResponseBody.Length > 0)
                {
                    ...
                    FeedResponseCore<dynamic> feedResponse = new FeedResponseCore<dynamic>(
                        feedResource,
                        itemCount,
                        response.Headers,
                        true,
                ...
                {
                    return new FeedResponseCore<TResult>(
